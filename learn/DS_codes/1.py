...
class doublyLinkedList:
    ...
    def __init__(self):
        ...
        self.__size = 0
        self.mem = Singleton.getInstance()
        self.__deleted = False
        
    
    def __Node(self, prev, element, next):
        index = self.mem.allocate()
        self.mem.memory[index] = prev
        self.mem.memory[index + 1] = element
        self.mem.memory[index + 2] = next
        return index
    ...
    def __set_prev(self, p, prev):
        self.mem.memory[self.__prev(p)] = prev

    def __set_element(self, p, element):
        self.mem.memory[self.__element(p)] = element

    def __set_next(self, p, next):
        self.mem.memory[self.__next(p)] = next

    def __get_prev(self, p):
        return self.mem.memory[self.__prev(p)]  

    def __get_element(self, p):
        return self.mem.memory[self.__element(p)]

    def __get_next(self, p):
        return self.mem.memory[self.__next(p)]
    ...
    def delete(self):
        self.erace()
        self.mem.free(self.__header)
        self.mem.free(self.__trailer)
