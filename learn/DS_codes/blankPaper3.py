class Memory:
    ...
    def pMemory(self):
        ...
        for i in range(0, self.MaxMemoryUsed+1, 3):
            ...
            print('({0}:{1} , {2}:{3} , {4}:{5}) -> '.format(str(i).rjust(4, ' '), str(self.memory[i]).rjust(self.outPutFormating, ' '), \
...
m = Memory()                    #should think about this one, tried using singleton, but not working
